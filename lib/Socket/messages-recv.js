...
const makeMessagesRecvSocket = (config) => {
  ...
  // ⚡ OPTIMIZATION 1: Aggressive Cache Settings - Faster Lookups
  const msgRetryCache =
    config.msgRetryCounterCache ||
    new node_cache_1.default({
      stdTTL: 600, // REDUCED: 3600 -> 600 (10 mins instead of 1 hour)
      useClones: false,
      max: 5000, // REDUCED: 10000 -> 5000
      checkperiod: 60, // NEW: Auto cleanup every 60s
    });
  const callOfferCache =
    config.callOfferCache ||
    new node_cache_1.default({
      stdTTL: 300, // OPTIMIZED: Same time
      useClones: false,
      max: 500, // REDUCED: 1000 -> 500
      checkperiod: 30, // NEW: Faster cleanup
    });
  const placeholderResendCache =
    config.placeholderResendCache ||
    new node_cache_1.default({
      stdTTL: 600, // OPTIMIZED: Aligned with msgRetryCache
      useClones: false,
      max: 2500, // REDUCED: 5000 -> 2500
      checkperiod: 45, // NEW: Auto cleanup
    });
  let sendActiveReceipts = false;

  // Debounce identity-change session refreshes per JID to avoid bursts
  const identityAssertDebounce = new node_cache_1.default({
    stdTTL: 2, // REDUCED: 5 -> 2 seconds
    useClones: false,
    max: 300, // REDUCED: 1000 -> 300
    checkperiod: 30, // NEW: Aggressive cleanup
  });
  ...
  // ⚡ OPTIMIZATION 2: Fast Retry with Early Exit
  ...
  const handleGroupNotification = (participant, child, msg) => {
    ...
    switch (child === null || child === void 0 ? void 0 : child.tag) {
      ...
      case "revoked_membership_requests":
        ...
        break;
      default:
      // console.log("Thehackingguard-DEBUG:", JSON.stringify({ ...child, content: Buffer.isBuffer(child.content) ? child.content.toString() : child.content, participant }, null, 2))
    }
  };
        ...
        processingMutex.mutex(async () => {
          ...
          try {
            ...
            if (errStr.includes("Bad MAC") || errStr.includes("Invalid MAC")) {
              logger.warn(
                { msgId: node.attrs.id, from: node.attrs.from },
                "⚠️ Bad MAC detected (Node 20/24 issue) - Forcing Retry..."
              );
              msg.messageStubType =
  ...
  // ⚡ OPTIMIZATION 3: Offline Node Processor with Batch Processing
    ...
    const enqueue = (type, node) => {
      ...
      const promise = async () => {
        while (nodes.length && ws.isOpen) {
          // ⚡ BATCH PROCESS: Handle 15 nodes at a time
          const batch = nodes.splice(0, 15);
          await Promise.allSettled(
            batch.map(async ({ type, node }) => {
